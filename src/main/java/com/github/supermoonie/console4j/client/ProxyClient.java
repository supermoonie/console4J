 ...
 */
@Slf4j
public class ProxyClient implements ConsoleContext {

    private ConsoleContext.ConnectionState connectionState = ConsoleContext.ConnectionState.DISCONNECTED;
    ...
    private ProxyClient.SnapshotMBeanServerConnection server = null;
     ...
     *
     * @throws UnsupportedOperationException If this {@code ProxyClient}
    ...
    private void setConnectionState(ConsoleContext.ConnectionState state) {
        ConsoleContext.ConnectionState oldState = this.connectionState;
        this.connectionState = state;
    }

    @Override
    public ConsoleContext.ConnectionState getConnectionState() {
    ...
    void connect(boolean requireSSL) {
        setConnectionState(ConsoleContext.ConnectionState.CONNECTING);
        try {
            tryConnect(requireSSL);
            setConnectionState(ConsoleContext.ConnectionState.CONNECTED);
        } catch (Exception e) {
            log.error(e.getMessage(), e);
            setConnectionState(ConsoleContext.ConnectionState.DISCONNECTED);
        }
    }
    ...
    private void tryConnect(boolean requireRemoteSSL) throws IOException {
        if (jmxUrl == null && "localhost".equals(hostName) && port == 0) {
            ...
            this.server = ProxyClient.Snapshot.newSnapshot(mbsc);
        } else {
    ...
    public String toString() {
        if (!isConnected()) {
            return "not connected " + displayName;
        } else {
            return displayName;
        }
    }
    ...
    public static class Snapshot {
        ...
        public static ProxyClient.SnapshotMBeanServerConnection
        newSnapshot(MBeanServerConnection mbsc) {
            final InvocationHandler ih = new ProxyClient.SnapshotInvocationHandler(mbsc);
            return (ProxyClient.SnapshotMBeanServerConnection) Proxy.newProxyInstance(
                    ProxyClient.Snapshot.class.getClassLoader(),
    ...
    static class SnapshotInvocationHandler implements InvocationHandler {
        ...
        private Map<ObjectName, ProxyClient.SnapshotInvocationHandler.NameValueMap> cachedValues = newMap();
        ...
        private Object getAttribute(ObjectName objName, String attrName)
            ...
            final ProxyClient.SnapshotInvocationHandler.NameValueMap values = getCachedAttributes(
        ...
        private synchronized ProxyClient.SnapshotInvocationHandler.NameValueMap getCachedAttributes(
                ObjectName objName, Set<String> attrNames) throws
                InstanceNotFoundException, ReflectionException, IOException {
            ProxyClient.SnapshotInvocationHandler.NameValueMap values = cachedValues.get(objName);
            ...
            values = new ProxyClient.SnapshotInvocationHandler.NameValueMap();
